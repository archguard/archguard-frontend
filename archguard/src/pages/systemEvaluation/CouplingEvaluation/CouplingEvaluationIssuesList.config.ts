...
export const CouplingEvaluationIssuesConfigs: {
  ...
  CIRCULAR_DEPENDENCY: {
    title: "循环依赖",
    badSmellDescription: `在模块、包、类、方法之间总会存在相互调用及依赖的关系，当两者或多者之间的依赖关系形成了一个闭环（如A依赖B、B依赖C、 C依赖A），则被识别为循环依赖，也可以称之为循环引用。这循环中的某一点产生不稳定变化，都可能会导致整个环产生不稳定变化。从软件设计的角度来看，最大的问题是相互依赖的模块之间的紧密耦合，这会导致单个模块的单独重用成为不可能。`,
    suggestion: `当各层级模块之间发生循环依赖，往往标示着他们在某方面有紧密耦合的需求，此时应该回顾看模块的划分是否合理：
  · 考虑是否应该把循环依赖的多方进行一定的合并

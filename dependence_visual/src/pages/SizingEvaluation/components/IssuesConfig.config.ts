import { methodColumns, classColumnsByMethodCount, classColumnsByLines, hubColumns, packageColumns, moduleColumns, dataClumpsColumns, deepInheritenceColumns } from './IssuesColumns.config';
...
export const IssuesConfigs: {
  ...
  DATA_CLUMPS: {
    title: '数据泥团',
    badSmellDescription: '关联关系弱的方法与属性被放置在同一个类中。如方法A仅使用了属性A，方法B仅使用了属性B，两者并无交集但同时出现在一个类中。SRP单一责任原则指出，一个类不应该有一个以上的变更理由。',
  ...
  DEEP_INHERITENCE: {
    title: '过深继承',
    badSmellDescription: '深度继承层次结构可以产生极其灵活的系统。不幸的是，与此同时，系统的可理解性和继承层次结构的适应性也会受到影响。如果继承级别太深，将几乎不可能通过读取代码来确定调用了哪个方法的实现。',

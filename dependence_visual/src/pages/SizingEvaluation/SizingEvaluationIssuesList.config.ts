...
export const SizingEvaluationIssuesConfigs: {
  ...
  METHOD: {
    ...
    suggestion:
      "拆分方法，寻找长方法内部的代码边界并将其逐一抽取为私有方法。寻找代码边界的关键找循环，找判断，找注释，找重复代码。另，在拆分方法改造时，应注意测试保护，尽可能先为原方法添加相应的单元测试/集成测试案例。并且，充分使用编辑器内置的重构菜单进行自动重构（如Intellij的Refactor-> Extract Method)，从而避免人手操作导致的失误。",
    tableConfigs: [
      {
  ...
  CLASS: {
    ...
    suggestion: `拆分类。如识别特性依恋，把相关的代码移动到另一个更依恋的类里。（如针对某对象的运算，可以把该运算逻辑移动到该对象内部），识别共性/特性方法，把相关方法抽取到接口、父类或子类中。把业务关联较少的部分，拆解到不同的类中处理。把重复代码抽取到公共方法或公共类中`,
    tableConfigs: [
      {

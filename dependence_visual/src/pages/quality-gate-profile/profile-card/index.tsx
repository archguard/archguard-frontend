...
import { Profile, ProfileConfig } from '..';
    ...
    if (editing) {
      return (
        <div className="btn-group">
          <Button type="link" danger onClick={() => onCancel()}>取消</Button>
          <Button type="link" disabled={!name} onClick={() => onSubmitClick()}>保存</Button>
        </div>
      )
    }
    return (
  ...
  return isEmpty ? (
    ...
    <Card className="profile-card" size="small" title={getTitle()} extra={getExtraButton()}>
      <ProfileCardContent
        ...
        editing={editing!}
        data={data}
        onFinish={submitData}></ProfileCardContent>
    </Card>
  )
}

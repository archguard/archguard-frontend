...
export const CouplingEvaluationIssuesConfigs: {
  ...
  DATA_CLUMPS: {
    title: "数据泥团",
    badSmellDescription: `关联关系弱的方法与属性被放置在同一个类中。如方法A仅使用了属性A，方法B仅使用了属性B，两者并无交集但同时出现在一个类中。
SRP单一责任原则指出，一个类不应该有一个以上的变更理由。`,
  ...
  CIRCULAR_DEPENDENCY: {
    title: "循环依赖",
    badSmellDescription: `在模块、包、类、方法之间总会存在相互调用及依赖的关系，当两者或多者之间的依赖关系形成了一个闭环（如A依赖B、B依赖C、 C依赖A），则被识别为循环依赖，也可以称之为循环引用。
这循环中的某一点产生不稳定变化，都可能会导致整个环产生不稳定变化。从软件设计的角度来看，最大的问题是相互依赖的模块之间的紧密耦合，这会导致单个模块的单独重用成为不可能。`,

...
import {
  queryScannerConfig,
  updateScannerConfig,
} from "api/scanner/ScannerConfig";
...
export default class ScanToolsConfig extends React.Component {
  ...
  renderToolItems(configs) {
    ...
    for (let tool in tools) {
      const available =
        tools[tool][tools[tool].findIndex((item) => item.key === "available")]
          .value;
      toolList.push({
        type: tool,
        available,
      ...
      <Fragment>
        {toolList.map((tool) => {
          return (
            <div key={tool.type}>
              <Card
                title={<div>{tool.type}{this.renderHelp(tool.type)}</div>}
                extra={
                  <Switch
                ...
                <div>
                  {tool.configs.map((configItem) => (
                    <Input
                      ...
                      onChange={(e) =>
                        this.onChange(tool.type, configItem.key, e.target.value)
                      }
                    ></Input>
                  ))}
                </div>

...
export default function setRadarOption(data) {
  ...
  const dataGroupByState = lodash.groupBy(
    transformedData,
    (item) => item.state
  );

  const dd = Object.keys(dataGroupByState).map((item) => {
    const value = lodash.groupBy(
      dataGroupByState[item],
      (subItem) => subItem.dimensionIndex
    );
    return {
      name: item,
